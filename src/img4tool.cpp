...
void tihmstar::img4tool::printMANB(const void *buf, size_t size,
...

  {
    ...
    for (auto &tag : sequence) {
      switch (++i) {
      case 0:
        assert(tag.getStringValue() == "MANB");
        printf(": MANB: ------------------------------\n");
        break;
      case 1: {
        ...
        if (printAll) {
          ...
          for (auto &selem : tag) {
            ...
            size_t privElem = 0;
            ASN1DERElement subsequence = parsePrivTag(
                selem.buf(),
                size - (size_t)((uint8_t *)selem.buf() - (uint8_t *)buf),
                &privElem);
...
void tihmstar::img4tool::printMANP(const void *buf, size_t size) {
...

  {
    ...
    for (auto &tag : sequence) {
      switch (++i) {
      case 0:
        assert(tag.getStringValue() == "MANP");
        printf(": MANP: ------------------------------\n");
        break;
      case 1: {
        assert(tag.tag().isConstructed);
        ...
        for (auto &elem : tag) {
          ...
          ASN1DERElement subsequence = parsePrivTag(
              elem.buf(),
              size - (size_t)((uint8_t *)elem.buf() - (uint8_t *)buf),
              &privElem);
          putStr((char *)&privElem, 4);

          assert(subsequence.tag().isConstructed);
          assert(subsequence.tag().tagNumber == ASN1DERElement::TagSEQUENCE);
          assert(subsequence.tag().tagClass ==
                 ASN1DERElement::TagClass::Universal);
...
ASN1DERElement tihmstar::img4tool::parsePrivTag(const void *buf, size_t size,
                                                size_t *outPrivTag) {
  size_t privTag = 0;
  ASN1DERElement::ASN1PrivateTag *ptag = NULL;
  ASN1DERElement::ASN1Len *tlen = NULL;
  size_t taginfoSize = 7;
  assert(size >= taginfoSize);
  assert(*(uint8_t *)buf == ASN1DERElement::TagPrivate);
